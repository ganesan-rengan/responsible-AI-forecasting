...
def evaluate_country(country_name, country_data):
    ...
    # ── ARIMA ─────────────────────────────────────────────────────────────────
    try:
        m = auto_arima(y_train, seasonal=False,
                       trace=False,
                       error_action='ignore',
    ...
    try:
        ...
        results['SARIMA_MAE'] = np.nan
        results['SARIMA_RMSE'] = np.nan
    ...
    if order is not None:
        try:
            ...
            results['SARIMAX_MAE'] = np.nan
            results['SARIMAX_RMSE'] = np.nan
    ...
    try:
        ...
        pred = scaler.inverse_transform(
            lstm_model.predict(X_te, verbose=0)
        )
        ...
        results['LSTM_MAE'] = np.nan
        results['LSTM_RMSE'] = np.nan
...
fig.suptitle(
    'Model Comparison – Average Error Across All Countries',
    fontsize=15,
    fontweight='bold',
    y=1.02
)
...
print("Plot saved → results/rmse_comparison.png")
